...
namespace GitCredentialManager
{
    public static class StreamExtensions
    {
        ...
        private static void WriteKeyValuePair(this TextWriter writer, KeyValuePair<string, IList<string>> kvp)
        {
            List<string> values = NormalizeValueList(kvp.Value).ToList();
            switch (values.Count)
            {
                case 0:
                    return;
                ...
                case 1:
                    WriteKeyValuePair(writer, kvp.Key, kvp.Value[0]);
                    return;
        ...
        private static IEnumerable<string> NormalizeValueList(IEnumerable<string> values)
        {
